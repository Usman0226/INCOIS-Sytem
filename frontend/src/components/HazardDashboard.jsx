      ...
      attachments: [
        {
          type: "text",
          content:
            "Strong sulfur smell before receding observed by multiple people.",
        },
        {
          type: "image",
          url: "https://picsum.photos/id/1011/600/360",
          mime: "image/jpeg",
        },
        {
          type: "video",
  ...
  },
  {
    ...
    userReport: {
      ...
      attachments: [
        {
          type: "image",
          url: "https://picsum.photos/id/1015/600/360",
          mime: "image/jpeg",
        },
      ],
    },
    ...
    hazards.forEach((h) => {
      ...
      marker.bindPopup(
        `<div>
          ...
          <p class="text-xs">${h.severity} • ${new Date(
          h.createdAt
        ).toLocaleString()}</p>
    ...
    if (att.type === "audio" && att.url) {
      return (
        ...
        >
          ...
          <div className="px-1 pt-1 text-xs text-slate-600">
            {att.mime || "audio"}
          </div>
        </div>
      );
    ...
    <div className="fixed inset-0 z-50">
      ...
      <div className="absolute right-0 top-0 h-full w-full max-w-md bg-white shadow-xl flex flex-col">
        <div className="px-4 py-3 border-b border-slate-200 flex items-center justify-between">
          ...
          <button
            className="px-3 py-1 bg-slate-200 text-slate-800 rounded hover:bg-slate-300 transition-colors font-medium"
            onClick={onClose}
          >
            Close
          </button>
        ...
        <div className="p-4 overflow-y-auto space-y-4">
          <div>
            ...
            <div className="text-sm text-slate-700 mb-1">
              Status: {hazard.status} • Zone: {hazard.zone}
            </div>
            <div className="text-sm text-slate-700 mb-1">
              Raised: {new Date(hazard.createdAt).toLocaleString()}
            </div>
            {hazard.radiusMeters && (
              <div className="text-sm text-slate-700 mb-1">
                Range: {(hazard.radiusMeters / 1000).toFixed(0)} km
              </div>
            )}
            <div className="text-sm text-slate-700">
              Coords: {hazard.lat.toFixed(2)}, {hazard.lng.toFixed(2)}
            </div>
          </div>
            ...
            {r ? (
              <>
                ...
                <div className="text-sm text-slate-800">
                  ...
                  {new Date(r.raisedAt).toLocaleString()}
                </div>
                {r.notes && (
                  <div className="text-sm text-slate-800">
              ...
              <div className="text-sm text-slate-600 italic">
                No user-submitted details available.
              </div>
            )}
          </div>
        </div>
...
export default function HazardDashboard({ scientist, onLogout }) {
  ...
  const filtered = useMemo(() => {
    return hazards.filter((h) => {
      ...
      if (filter.severities.size && !filter.severities.has(h.severity))
        return false;
...
const verify = async (id, e) => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-100">
      {/* Header */}
      <header className="bg-sky-100 px-4 md:px-6 py-0 flex items-center justify-between border-b border-slate-200">
        <div className="flex items-center gap-2">
          {/* Mobile toggles */}
          <button
            className="md:hidden px-2 py-1 bg-slate-200 rounded"
            onClick={() => setLeftOpen(true)}
            aria-label="Open filters"
          >
            Filters
          </button>
          <h1 className="font-bold text-base md:text-lg text-slate-700">
            <div className="flex">
              <img
                className="w-16 h-16 m-3 object-contain"
                src={logo}
                alt="here is a img"
              />
              <p className="mt-3 text-s">DashBoard</p>
            </div>
          </h1>
        </div>
        <div className="flex items-center gap-2 md:gap-3 text-sm text-slate-700">
          ...
          <button
            ...
            className="md:hidden px-3 py-1 bg-slate-200 text-slate-800 rounded hover:bg-slate-300 transition-colors font-medium"
          ...
          {scientist && (
            <div className="flex items-center gap-3">
              <span className="text-sm text-slate-700 font-medium">
                Welcome, {scientist.name || scientist.email}
              </span>
              <button
                onClick={onLogout}
      ...
      >
        ...
        <aside
          className={`bg-white border-r overflow-y-auto p-4 ${
            leftOpen ? "fixed inset-0 z-40" : "hidden"
          } md:block md:relative md:h-full [scrollbar-width:none] [&::-webkit-scrollbar]:hidden`}
        >
          {/* Mobile close */}
          ...
          <div className="space-y-4">
            <div>
              ...
              {SEVERITIES.map((s) => (
                <button
                  key={s}
                  className={`block w-full text-left px-3 py-2 mb-1 rounded text-slate-180 font-medium hover:bg-slate-200 transition-colors ${
                  ...
                  onClick={() =>
                    setFilter((prev) => ({
                      ...prev,
                      severities: toggleSet(prev.severities, s),
                    }))
                  }
                >
                  {s}
              ...
              {STATUSES.map((s) => (
                <button
                  ...
                  onClick={() =>
                    setFilter((prev) => ({
                      ...prev,
                      statuses: toggleSet(prev.statuses, s),
                    }))
                  }
                >
                  {s}
        ...
        <main className="relative md:h-full md:overflow-hidden">
          <div style={{ height: "100%" }} className="h-full relative z-0">
            ...
            >
              ...
              {showPins && (
                <MarkersClusterGroup hazards={filtered} iconFor={iconFor} />
              )}
            </MapContainer>
          </div>
          ...
          <div className="md:hidden absolute bottom-2 left-1/2 -translate-x-1/2 bg-white/95 backdrop-blur px-4 py-3 rounded-lg shadow-lg flex items-center gap-3 z-10 border border-slate-200">
            <button
              className="px-3 py-1 bg-slate-200 text-slate-800 rounded font-medium hover:bg-slate-300 transition-colors"
        ...
        <aside
          className={`bg-white border-l overflow-y-auto p-4 ${
            rightOpen ? "fixed inset-0 z-40" : "hidden"
          } md:block md:relative md:h-full`}
        >
          <div className="md:hidden flex justify-between items-center mb-4">
            <div className="font-semibold text-slate-180 text-lg">
              Validation Queue
            </div>
            <button
              onClick={() => setRightOpen(false)}
              className="px-3 py-1 bg-slate-200 text-slate-180 rounded hover:bg-slate-300 transition-colors font-medium"
            >
              Close
            </button>
          </div>

          <h3 className="font-semibold mb-4 hidden md:block text-slate-800 text-lg">
            Validation Queue
          </h3>
          {pending.map((q) => (
            <div
            ...
            >
              ...
              <div className="text-sm text-slate-700 mb-2">
                {q.type} • {new Date(q.createdAt).toLocaleString()}
              </div>
